...
pong.prototype.registerPlayer = function(player_name, options) [
	var data = {
...
pong.prototype.registerPlayers = function(player_names) {
	return Q.all(player_names.map(pong.registerPlayer));
}

pong.prototype.findPlayer = function(player_name) {
	var q = null;
...
pong.prototype.findPlayers = function(player_names) {
	return Q.all(player_names.map(pong.findPlayer));
}

pong.prototype.updateWins = function(player_names) {
...
pong.prototype.updateLosses = function(player_names) {
	return pong.findPlayers(player_names).then(function(players) {
		return Q.all(players.map(function(player) {
			player.losses++;
			return player.save();
...
pong.prototype.checkChallenge = function(player_names) {
	return this.findPlayers(player_names).then(function(players) {
		return Q.all(players.map(function(player) {
			var deferred = Q.defer();
			...
			Challenge.findOne({ _id: player.currentChallenge }).then(function(challenge) {
				if (challenge) {
					deferred.reject(
						new Error("There's already an active challenge between " +
							challenge.challenger.join(' and ') + ' and ' +
							challenge.challenged.join(' and ') + '.'));
				} else {
					deferred.resolve(player);
				}, function(err) {
					deferred.reject(err);
...
pong.prototype.setChallenge = function(player_names, challenge_id) {
	return this.findPlayers(player_names).then(function(players) {
		return Q.all(players.map(function(player) {
...
pong.prototype.ensureUniquePlayers = function(players) {
	return this.findPlayers(players).then(function(players) {
...
pong.prototype.createSingleChallenge = function(c1, c2) {
	var _this = this;

	return this.ensureUniquePlayers([c1, c2]).then().spread(function(c1, c2) {
		return _this.checkChallenge([c1, c2]).then(function() {
			return new Challenge({
				...
				return _this.setChallenge([c1, c2], challenge._id).then(function() {
					var deferred = Q.defer();
...
pong.prototype.createDoubleChallenge = function(c1, c2, c3, c4) {
	var _this = this;

	return this.ensureUniquePlayers([c1, c2, c3, c4]).then().spread(function(c1, c2, c3, c4) {
		return _this.checkChallenge([c1, c2, c3, c4]).then(function() {
			return new Challenge({
				state: 'Proposed',
			       type: 'Doubles',
			       date: Date.now(),
				...
				return _this.setChallenge([c1, c2, c3, c4], challenge._id).then(function() {
					var deferred = Q.defer();
...
pong.prototype.acceptChallenge = function(player_name) {
}
...
pong.prototype.declineChallenge = function(player_name) {
}
...
pong.prototype.chickenChallenge = function(player_name) {
}
...
pong.prototype.calculateTeamElo = function(p1, p2) {
}
...
pong.prototype.eloSinglesChange = function(winner_name, loser_name) {
}
...
pong.prototype.eloDoublesChange = function(p1, p2, p3, p4) {
}
...
pong.prototype.win = function(player_name) {
}
...
pong.prototype.lose = function(player_name) {
}
...
pong.prototype.resetAll = function() {
}
...
pong.prototype.getDuelGif = function(cb) {
}
