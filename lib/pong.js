...
var pong = {
  ...
  checkChallenge: function (player_names) {
    return pong.findPlayers(player_names).then(function(players) {
      return Q.all(players.map(function (player) {
        var deferred = Q.defer();
        Challenge.findOne({ _id: player.currentChallenge, state: 'Finished' }).then(function (challenge) {
          if (challenge && challenge.state != 'Finished') {
  ...
  acceptChallenge: function (player_name) {
    return findChallengeForPlayer(player_name).then(function (challenge) {
      var deferred = Q.defer();
    ...
    return findChallengeForPlayer(player_name).then(function (challenge) {
      ...
      if (challenge && challenge.state == 'Proposed') {
        if (player_name == challenge.challenger[0]) {
          ...
          saveChallengeAndUpdatePlayer(challenge).then(function () {
  ...
  chickenChallenge: function (player_name) {
    function chickenedMessageText(player_name, who_challenge_is_against) {
      return player_name + ' chickened out of the challenge against ' + who_challenge_is_against.join(' and ') + ".";
    }
    ...
    return findChallengeForPlayer(player_name).then(function (challenge) {
      ...
      if (challenge && challenge.state == 'Proposed') {
        if (player_name == challenge.challenger[0]) {
          ...
          saveChallengeAndUpdatePlayer(challenge).then(function () {
            deferred.resolve({ message: chickenedMessageText(player_name, challenge.challenged), challenge: challenge });
          });
        } else {
        ...
        challenge.state = 'Chickened';
        saveChallengeAndUpdatePlayer(challenge).then(function () {
          var who_challenge_is_against =
            (player_name == challenge.challenger[0] || player_name == challenge.challenger[1]) ?
            challenge.challenged : challenge.challenger;
          deferred.resolve({ message: chickenedMessageText(player_name, who_challenge_is_against), challenge: challenge });
        });
      } else {
...
function findChallengeForPlayer(player_name) {
  return pong.findPlayer(player_name).then(function(player) {
    return Challenge.findOne({ _id: player.currentChallenge });
  });
}

function saveChallengeAndUpdatePlayer(challenge) {
  return Q.when(challenge.save(), function (challenge) {
    Player.update({ currentChallenge: challenge._id }, { currentChallenge: null }, { multi: true });
  });
}
