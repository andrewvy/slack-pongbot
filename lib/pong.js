...
var pong = {
  ...
  findPlayers: function(user_names, cb) {
    var findPlayer = function(user_name) {
      var deferred = Q.defer();
      pong.findPlayer(user_name, function (err, player) {
        if (err) {
          deferred.reject(err);
        } else {
  ...
  updateWins: function (users, cb) {
    var updateWins = function(user_name) {
      var deferred = Q.defer();
      pong.findPlayer(user_name, function (err, user) {
        if (user) {
          ...
          deferred.reject(err || new Error("User '" + user_name + "' does not exist."), null);
        }
      });
      return deferred.promise;
    };
  ...
  updateLosses: function (users, cb) {
    var updateLosses = function(user_name) {
      var deferred = Q.defer();
      pong.findPlayer(user_name, function (err, user) {
        if (user) {
          user.losses++;
  ...
  ensureNoChallenges: function(user_names, cb) {
    var findChallenge = function(user_name) {
      var deferred = Q.defer();
      pong.findChallenge(user_name, function (err, challenge) {
        if (challenge || err) {
          deferred.reject(err || new Error("There's already an active challenge between " + challenge.challenger.join(' and ') + ' and ' + challenge.challenged.join(' and ') + '.'));
        } else {
          deferred.resolve();
        }
      });
  ...
  setChallenge: function(user_names, challenge_id, cb) {
    var setChallenge = function(user_name) {
      var deferred = Q.defer();
      pong.findPlayer(user_name, function (err, user) {
        if (user) {
          user.currentChallenge = challenge_id;
          ...
          deferred.reject(err || new Error("User '" + user_name + "' does not exist."));
        }
      });
      return deferred.promise;
    };
  ...
  createSingleChallenge: function (challenger_name, challenged_name, cb) {
    pong.ensureUniquePlayers([challenger_name, challenged_name], function (err) {
      ...
      pong.findChallenge(challenger_name, function (err, challenge) {
        pong.ensureNoChallenges([challenger_name, challenged_name], function (err) {
          if (err) return cb(err, null);
          new Challenge({
            state: 'Proposed',
            type: 'Singles',
            ...
            if (err) return cb(err, null);
            pong.setChallenge([challenger_name, challenged_name], challenge._id, function(err) {
              if (err) return cb(err, null);
              cb({ message: challenger_name + ' has challenged ' + challenged_name + ' to a ping pong match!' }, challenge);
            });
          });
        });
      });
  ...
  createDoubleChallenge: function (c1, c2, c3, c4, cb) {
    pong.ensureUniquePlayers([c1, c2, c3, c4], function (err) {
      if (err) return cb(err, null);
      pong.ensureNoChallenges([c1, c2, c3, c4], function (err) {
        if (err) return cb(err, null);
        new Challenge({
          state: 'Proposed',
          type: 'Doubles',
          ...
          if (err) return cb(err, null);
          pong.setChallenge([c1, c2, c3, c4], challenge._id, function(err) {
            if (err) return cb(err, null);
            cb({ message: c1 + ' and ' + c2 + ' have challenged ' + c3 + ' and ' + c4 + ' to a ping pong match!' }, challenge);
          });
        });
      });
    });
    ...
    pong.findChallenge(user_name, function (err, challenge) {
      if (err || !challenge) {
        ...
        challenge.save(function (err) {
          ...
          cb({ message: user_name + ' accepted ' + challenge.challenger[0] + "'s challenge." }, challenge);
        });
      ...
      if (err || !challenge) {
        ...
        challenge.save(function (err) {
          ...
          Player.update({ currentChallenge: challenge._id }, { currentChallenge: null }, { multi: true }, function (err) {
            if (err) return cb(err, challenge);
            cb({ message: user_name + ' declined ' + challenge.challenger[0] + "'s challenge." }, challenge);
          });
        });
      } else {
  ...
  calculateTeamElo: function (p1, p2, cb) {
    pong.findPlayers([p1, p2], function(err, players) {
      if (err) return cb(err, players);
      cb(null, (players[0].elo + players[1].elo) / 2);
    });
  },

  eloSinglesChange: function (winner_name, loser_name, cb) {
    pong.findPlayers([winner_name, loser_name], function(err, players) {
      if (err) return cb(err, players);
  ...
  eloDoublesChange: function (p1, p2, p3, p4, cb) {
    pong.calculateTeamElo(p1, p2, function (err, t1) {
      ...
      pong.calculateTeamElo(p3, p4, function (err, t2) {
        ...
        pong.findPlayers([p1, p2, p3, p4], function(err, players) {
          if (err) return cb(err, players);
          ...
          Player.update({ currentChallenge: challenge._id }, { currentChallenge: null }, { multi: true }, function (err) {
            ...
            if (challenge.type === 'Doubles') {
              ...
              pong.eloDoublesChange(winners[0], winners[1], losers[0], losers[1], function(err) {
                if (err) return cb(err, challenge);
                pong.updateWins(winners, function (err) {
                  if (err) return cb(err, challenge);
                  pong.updateLosses(losers, function (err) {
                    if (err) return cb(err, challenge);
                    cb({ message: 'Match has been recorded, ' + winners.join(' and ') + ' have defeated ' + losers.join(' and ') + '.' });
                  });
                });
              });
              ...
              pong.eloSinglesChange(winner, loser, function(err) {
                ...
                pong.updateWins([winner], function (err) {
                  ...
                  pong.updateLosses([loser], function (err) {
                    if (err) return cb(err, challenge);
                    cb({ message: 'Match has been recorded, ' + winner + ' has defeated ' + loser + '.' });
                  });
                });
              });
            }
  ...
  ensureUniquePlayers: function(players, cb) {
    ...
    Object.keys(playerCounts).forEach(function (key, index, array) {
      ...
      if (count > 1) {
        hasDuplicates = true;
        cb(new Error('Does ' + key + ' have ' + 2 * count + ' hands?'), players);
      } else if (!hasDuplicates && (index === array.length - 1)) {
        cb(null, players);
      }
    });
  }
