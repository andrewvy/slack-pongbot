...
var pong = {
				...
				pong.checkChallenge(challenged, function(y2) {
					if (y2 === false) {
						...
						c.save( function(err, nc) {
							...
							message = "You have challenged " + challenged + " to a ping pong match!";
							cb(message);
						});
					} else {
	...
	createOpenSinglesChallenge: function(user_name, cb) {
		pong.checkChallenge(user_name, function(y) {
			if (y === false) {
				if (!pong.currentChallenge) {
					var c = new Challenge({
							state: "Proposed",
							type: "Single",
							date: Date.now(),
					...
					c.save( function(err, nc) {
						if (err) return new Error(err);
						pong.setChallenge(user_name, nc._id);
						pong.currentChallenge = nc._id;
						message = user_name + " has created an open singles match! Type `pongbot join` to join!";
						cb(message);
					});
				} else {
					cb("There's already an open challenge! Join that one!");
				}
			} else {
				cb("You're already in a pending challenge.");
			}
		});
	},
	join: function(user_name, cb) {
		pong.checkChallenge(user_name, function(y) {
			if (y === false) {
				if (pong.currentChallenge) {
					pong.setChallenge(user_name, pong.currentChallenge);
					Challenge.findOne({ _id: y.currentChallenge }, function(err, c) {
						c.state = "Accepted";
						pong.currentChallenge = false;
						var rand = Math.floor(Math.random() * 2) + 1;
						if (rand === 1) {
							message = user_name + " accepted " + c.challenger[0] + "'s open challenge. " + c.challenger[0] + " serves first!";
						} else {
							message = user_name + " accepted " + c.challenger[0] + "'s open challenge." + user_name + " serves first!";
						}
						cb(message);
						c.save(function (err) {
							if (err) return handleError(err);
					...
					cb("There is no open challenge to join. Create one with `pongbot open singles`");
				}
			} else {
				cb("You're already in a pending challenge.")
			}
