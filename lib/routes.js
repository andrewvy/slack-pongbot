  ...
  if (hook) {
      ...
      pong.findPlayer(hook.user_name, function (err, user) {
        ...
        if (user) {
          if (params[2] == 'doubles') {
            ...
            pong.findPlayer(params[3], function (err, user) {
              if (user) {
    ...
    case 'reset':
      if (hook.user_name === 'vy') {
        pong.findPlayer(params[2], function (err, user) {
          if (err) {
            message = err.message + "\nYou're not registered! Use the command 'pongbot register' to get into the system.";
            res.json({ text: message });
          } else if (user) {
